...
describe("Passauth email flows", () => {
  ...
  describe("login", () => {
    test("blocks authentication when the email is not confirmed", async () => {
    ...
    test("authenticates when the email is confirmed", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
  ...
  describe("sendConfirmPasswordEmail", () => {
    test("sends a confirmation email with the generated link", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
    ...
    test("passes emailParams.linkParams to the confirmation link builder", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
      const linkParams = {
    ...
    test("throws when the email cannot be sent", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
  ...
  describe("confirmEmail", () => {
    ...
    test("calls repo.confirmEmail with default params", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
    ...
    test("passes emailParams to repo.confirmEmail", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
      const emailParams = {
        linkParams: {
    ...
    test("invalidates a token after it is used once", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
    ...
    test("invalidates the previous token when a new one is generated with the same key", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
      const confirmEmailSpy = jest.spyOn(
        emailHandlerConfig.services,
    ...
    test("keeps tokens isolated by key for the same email", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
      const confirmEmailSpy = jest.spyOn(
        emailHandlerConfig.services,
    ...
    test("defaults the token key to the email when key is not provided", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
      const confirmEmailSpy = jest.spyOn(
        emailHandlerConfig.services,
      ...
      await expect(
        sut.confirmEmail(userData.email, secondToken),
      ).resolves.toBeUndefined();
    });
  });

  describe("sendResetPasswordEmail", () => {
    test("sends the expected email payload", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
    ...
    test("passes emailParams.linkParams to the reset link builder", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
      const emailParams = {
        linkParams: {
  ...
  describe("confirmResetPassword", () => {
    test("returns success false when the token is invalid", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
    ...
    test("passes the expected params to repo.resetPassword", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
    ...
    test("passes emailParams to repo.resetPassword", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
      const emailParams = {
        linkParams: {
  ...
  describe("register", () => {
    test("throws when the confirmation email cannot be sent", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
    ...
    test("sends a confirmation email after the user is registered", async () => {
      const passauth = Passauth(passauthConfig);
      const sut = passauth.handler;
